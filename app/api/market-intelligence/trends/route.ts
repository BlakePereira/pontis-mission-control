...
async function fetchTrends() {
  // Dynamic import since google-trends-api uses CommonJS
  ...
  for (const [group, keywords] of Object.entries(KEYWORD_GROUPS)) {
    try {
      const timeData = await googleTrends.interestOverTime({
        keyword: keywords.slice(0, 5), // Max 5 per request
  ...
  // Fetch geographic interest for top keyword
  try {
  ...
  // Fetch related queries for "headstone" and "digital memorial"
